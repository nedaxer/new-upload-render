...
export default function UnifiedAdminPortal() {
          ...
          if (data.type === 'PASSWORD_CHANGED') {
            ...
            if (selectedUser && selectedUser._id === data.userId) {
              ...
              showSuccessNotification(
                "Password Updated",
                "User password was changed - display cleared for security"
              );
            }
  ...
  const adminLoginMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showSuccessNotification(
        ...
        "Welcome to the unified admin portal"
      );
    },
    onError: (error: Error) => {
  ...
  const toggleDepositRequirementMutation = useMutation({
    ...
    onError: (error: any) => {
      showSuccessNotification(
        "Error Updating Deposit Requirement",
  ...
  const toggleWithdrawalAccessMutation = useMutation({
    ...
    onError: (error: any) => {
      showSuccessNotification(
        "Error Updating Withdrawal Access",
  ...
  const deleteUserMutation = useMutation({
    ...
    onSuccess: () => {
      showSuccessNotification(
        "Account Deleted",
        ", "User account permanently deleted from MongoDB",
  ...
  const toggleTransferAccessMutation = useMutation({
    ...
    onError: (error: any) => {
      showSuccessNotification(
        "Error Updating Transfer Access",
  ...
  const getUserPasswordMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      showSuccessNotification(
        "Password Key Retrieved",
        ", "User password key has been fetched for security review",
  ...
  const sendConnectionRequestMutation = useMutation({
    ...
    onSuccess: () => {
      showSuccessNotification(
        ...
        ", "Connection request sent to user successfully",
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (!adminCredentials.email || !adminCredentials.password) {
      showSuccessNotification(
        "Missing Information",
        ", "Please enter both your admin email and password to continue.",
  ...
  const handleAddFunds = () => {
    if (!selectedUser || !fundAmount) {
      showSuccessNotification(
        "Missing Information",
        ", "Please select a user and enter amount",
  ...
  const handleSendConnectionRequest = () => {
    if (!selectedUser) {
      showSuccessNotification(
        "No User Selected",
        ", "Please select a user to send connection request",
    ...
    if (!connectionRequestData.serviceName || !connectionRequestData.customMessage || !connectionRequestData.successMessage) {
      showSuccessNotification(
        "Missing Information",
        ", "Please fill in all connection request fields",
  ...
  const handleAdminRefresh = async () => {
    try {
      ...
      showSuccessNotification(
        "Data Refreshed",
        ", "Admin dashboard updated with latest data",
                            ...
                            <div className="flex items-center space-x-2 mt-2">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  showSuccessNotification( "Password Key Copied", ", "Full password key copied to clipboard for security review" });
                                }}
