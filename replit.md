...
Changelog:
- June 23, 2025. Optimized mobile app performance to prevent feature reloading: implemented comprehensive caching system with React Query configuration (disabled refetchOnMount, refetchOnWindowFocus, extended staleTime and gcTime), created global app state manager to persist user data, favorites, market data across navigation, added persistent state hooks for component state preservation, enhanced navigation cache to prevent component reinitialization, implemented intelligent market data caching to reduce API calls from every 10 seconds to 60 seconds with 30-second cache, fixed React Query cache invalidation patterns to use setQueryData instead of invalidateQueries, added chart state persistence to prevent TradingView widget reloading
