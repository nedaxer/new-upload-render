...
Changelog:
- June 28, 2025. Enhanced verification process UX flow improvements: moved all Next buttons to bottom of verification pages for better mobile experience, made back button on verification start page navigate to home page instead of closing modal, reduced progress bar height from h-2 to h-1 across all verification steps for smaller visual footprint, updated verification completion page to navigate directly to home page after submission, restructured all verification step components with flex layouts to properly position buttons at bottom while maintaining scrollable content areas
