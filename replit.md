...
Changelog:
- June 24, 2025. Implemented branded logo fallback system: created logo service that generates SVG logos for news sources when images fail to load, added real brand colors and styling for CryptoSlate, BeInCrypto, Google News, CoinDesk, CryptoBriefing, enhanced news articles to display actual news source logos instead of generic placeholders, improved visual consistency across all news sources with professional branding
...
- June 24, 2025. Enhanced pull-to-refresh with advanced animations: implemented scroll-distance controlled timing for smooth letter transitions, added Framer Motion animations with spring physics and easing, created distinctive orange gradient refresh header (50px) completely separate from logo header, added parallax effects and background animations, implemented smooth scroll-based resistance with cubic easing, enhanced visual feedback with drop shadows and brightness filters
