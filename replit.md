...
Changelog:
- June 21, 2025. Fixed API request parameter order causing application hangs: corrected apiRequest function parameter order from (method, url, data) to (url, method, data), added proper error handling and authentication checks to prevent API calls for unauthenticated users, resolved fetch errors that were causing the application to become unresponsive and slow
- June 21, 2025. Implemented persistent favorites and chart pair memory: added user favorites and preferences database tables, created favorites functionality with real-time database persistence, integrated chart pair memory to maintain selected trading pairs across sessions, connected home page crypto cards with navigation to trade page preserving pair selection, updated storage layer with MySQL-compatible favorites and preferences management for seamless user experience
