...
Changelog:
- June 26, 2025. Fixed mobile bottom navigation panel covering modals with comprehensive z-index restructuring: reduced bottom navigation z-index from 10001 to 1000 to allow modals to appear above it, completely recreated deposit modal with extremely high z-index (50000) to ensure visibility above navigation panel, removed duplicate PWA install prompt system from main.tsx that was creating conflicting popups, enhanced PWA install prompt with highest z-index (99999) and fallback display after 3 seconds for users without browser install support, implemented clean z-index hierarchy where deposit modal and install prompt now properly appear above bottom navigation without interference, eliminated duplicate install prompts that were showing both behind and above navigation
