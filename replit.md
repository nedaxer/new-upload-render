...
Changelog:
- June 25, 2025. Created secure admin system accessible at /mobile/admin with hidden admin login at /mobile/admin-login, implemented admin user account (robinstephen003@outlook.com with same password), built mobile-friendly admin dashboard with user search by email/UID, added virtual USD fund management and user account deletion features, includes proper admin authentication and access controls using MongoDB storage
