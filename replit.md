...
Changelog:
- June 25, 2025. Completely rebuilt clean admin portal system: removed all previous admin components and hardcoded users, created fresh admin portal at /admin-portal with modern gradient design, implemented new authentication (admin@nedaxer.com / NedaxerAdmin2025), added comprehensive user search by name/email/username/UID with real-time updates, integrated fund management with MongoDB database updates, built secure account deletion with admin protection, designed mobile-friendly interface with glassmorphism effects, fixed session persistence and authentication issues, added users overview panel with live user count and copy ID functionality, disabled splash screen for admin routes
