...
Changelog:
- June 21, 2025. Fixed critical authentication API parameter order bugs: corrected all apiRequest calls in authentication hooks and user profile functions to use proper (url, method, data) parameter order, resolved login/registration failures that were preventing user access to the platform, fixed authentication state management across the entire application
