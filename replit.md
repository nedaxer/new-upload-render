...
Changelog:
- June 26, 2025. Implemented single ultramarine blue theme system: converted theme context from multi-theme (blue/white/black) to single ultramarine blue theme (#1e40af), removed theme selector from settings page since only one theme remains, updated all chart background colors to use ultramarine blue instead of gray, updated tab selectors and UI elements to use consistent ultramarine color scheme, simplified theme utility functions to return fixed ultramarine colors for consistent branding across entire mobile application
