...
Changelog:
- June 28, 2025. Implemented comprehensive offline storage system for mobile app: created localStorage-based offline storage utility with automatic expiry management (news expires after 24 hours, crypto prices after 10 minutes), developed custom React hooks for seamless online/offline data fetching with fallback to local storage, enhanced notifications, news, transaction history, wallet summary, crypto prices, and favorites with offline capabilities, added offline indicators with WiFi off icons to notifications and assets history page headers, integrated automatic online/offline status monitoring with data refresh when back online, ensured complete app functionality when internet disconnected using cached data, transfer sender/receiver details now show only relevant party information (sender for received transfers, receiver for sent transfers)
