...
Changelog:
- June 28, 2025. Implemented comprehensive KYC verification system with progress tracking and status management: enhanced verification banner with step progress indicators (1-checkmark, 2-orange when questionnaire completed, 3-gray), added X close buttons to all verification step pages for easy exit, created new KYC Status page (/mobile/kyc-status) replacing old identity verification with detailed progress tracking, verification steps display, issue reporting, and action buttons, built Verification Submitted page (/mobile/verification-submitted) showing submission summary with under review status and next steps, updated verification flow to redirect to submitted page after completion instead of generic complete screen, modified settings page to redirect to new KYC status page, implemented dynamic verification banner visibility (hides when KYC submitted or verified), added comprehensive verification progress calculation based on completion steps, enhanced verification status tracking (none/pending/verified/rejected) with appropriate UI states and messaging
