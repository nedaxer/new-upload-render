...
Changelog:
- June 28, 2025. Enhanced verification flow with professional design and UX improvements: replaced hiker image with professional mountain climber illustration for first verification screen, implemented personalized greeting using user's full name ("Ekaterina, is it really you?"), removed all X close buttons from verification flow leaving only back navigation, eliminated automatic progression requiring users to tap "Next" button for each step, added loading states (600-800ms) between all verification steps for professional UX, maintained multi-screen questionnaire format with one question per screen, updated all green colors to orange throughout verification process, reduced font sizes for cleaner design, hidden bottom navigation during entire verification flow, ensured document upload maintains Front/Back layout for driving licenses and residence permits
