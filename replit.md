...
Changelog:
- June 26, 2025. Fixed splash screen and removed duplicate loading screens: restored gradient background to splash screen (from-gray-900 via-black to-gray-800), eliminated duplicate undesigned splash screen that appeared after original, removed all secondary loading screens from MobileAppLoader and AuthRedirect components, added "Thanks for choosing Nedaxer" text at bottom, removed all lighting effects and blur for crisp clear animation, made letters smaller (w-24 to w-12) and final logo smaller (w-80 to w-40), confirmed real-time BTC conversion working on home page showing USD balance equivalent in BTC using live CoinGecko prices
