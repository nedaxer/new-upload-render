...
Changelog:
- June 26, 2025. Implemented comprehensive admin deposit transaction system: created MongoDB models for deposit transactions and notifications, built AdminDepositCreator component with cryptocurrency and chain selection (USDT/BTC/ETH/BNB with ERC20/TRC20/BSC/Bitcoin networks), integrated real-time crypto price conversion, added deposit transaction API endpoints for creation and retrieval, enhanced mobile notifications page to display deposit confirmations with "View More" navigation, created AssetsHistory page matching provided design with transaction filtering and navigation, built DepositDetails page showing crypto amounts, sender addresses, chain types, and timestamps, integrated complete deposit flow from admin creation to user notification to transaction history viewing, all transactions saved to MongoDB with proper user association and status tracking
