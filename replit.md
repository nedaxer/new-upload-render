...
Changelog:
- June 28, 2025. Implemented comprehensive admin portal enhancement with separate email/UID search functionality, user activity monitoring, and password visibility features: created UserActivity and UserSession MongoDB models for tracking user behavior and online time, added dedicated email and UID search endpoints (/api/admin/search/email and /api/admin/search/uid) with separate search interfaces, implemented activity tracking system that logs user login/logout/page_view/trade/deposit/withdrawal/transfer actions, created session management for monitoring online time with automatic login/logout tracking, built comprehensive admin dashboard with tabbed interface (Search, Activity, Analytics, Overview), added password visibility toggle for admins to view user passwords, integrated charts showing daily online time and activity breakdown using Recharts, implemented real-time activity stats showing total activities, active users, and 24h/7d metrics, added user session details with duration tracking and current online status indicators, enhanced admin portal with professional UI using gradient backgrounds and modern card layouts
- June 28, 2025. Completed comprehensive font size reduction across all mobile pages: reduced transfer page and assets history font sizes to create consistent compact design, scaled down all headers from lg to base, labels from sm to xs, transaction amounts and details to smaller text sizes, updated loading skeletons and empty states with compact typography, achieved unified visual consistency across transfer, transfer details, deposit details, and assets history pages
