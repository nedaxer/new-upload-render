...
Changelog:
- June 23, 2025. Implemented zero-balance user system: removed all crypto assets from mobile home page, updated user registration to create only $0.00 USD balance instead of starter funds, modified wallet summary API to return only USD balance, added real-time BTC conversion display below main balance, reset all existing user balances to $0.00 to ensure fair start for all users
